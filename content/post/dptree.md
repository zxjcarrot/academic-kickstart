...
### In-Place Merge
在处理buffer合入base的过程中，一种显而易见的方式是采用LSMT的out-of-place的合并方式，但是我们发现这种合并方式可能会产生1/R的NVM写入放大，使得DPTree比传统NVM索引结构有更好的写入改善。实际上，LSMT的这种out-of-place合并方式是为block-storage设计的。在HDD/SSD这类无法进行按字节寻址的设备，这种方式可以非常好地利用按block为单位顺序写特性来提升性能。因此即使写放大严重，LSMT仍然可以很显著地提升整体性能。然而对于NVM来说，情况就变得不一样了。由于NVM可以按字节寻址，DPTree便利用了这种特性设计了一种in-place merge方式来降低写入量，减少写放大。
